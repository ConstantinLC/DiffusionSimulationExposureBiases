...
def evaluate_dw_train_inf_gap(models, val_loader, device):
    ...
    with torch.no_grad():
        ...
        mse_input_all = {name : [] for name in models}
        #mse_input_type2_all = {name : [] for name in models}
        shortcuts_all = {name: [] for name in models}
        ...
        for batch_idx, sample in enumerate(val_loader):
            ...
            for name in models:
                ...
                for ts in range(0, model.timesteps):
                    for prev_ts in range(ts, model.timesteps):
                        input_data = x0_estimates[model.timesteps-1-prev_ts]
                        _, _, x0_estimate = model(conditioning=conditioning_frame, data=input_data, return_x0_estimate = True, 
                         lower_limit_timesteps_training=ts, upper_limit_timesteps_training=ts+1)
                        ratio = torch.mean((x0_estimate - target_frame)**2)/torch.mean((x0_estimates_clean[model.timesteps-1-ts] - target_frame)**2)
                        if ratio > 1.04:
                            print(model.sqrtAlphasCumprod.ravel()[ts], model.sqrtAlphasCumprod.ravel()[prev_ts])
                            mse_input.append(torch.mean((x0_estimates_clean[model.timesteps-1-prev_ts] - target_frame)**2))
                            shortcuts.append(prev_ts)
                            break
                        if ratio <= 1.04 and prev_ts == model.timesteps-1:
                            mse_input.append(torch.mean((x0_estimates_clean[model.timesteps-1-prev_ts] - target_frame)**2))
                            shortcuts.append(prev_ts)
                    
                    """for i, error in enumerate(list(torch.linspace(1, 50, 50))):
                        print(i, error)
                        input_data = target_frame + (x0_estimates[model.timesteps-2-ts] - target_frame)*error
                        _, _, x0_estimate = model(conditioning=conditioning_frame, data=input_data, return_x0_estimate = True, 
                         lower_limit_timesteps_training=ts, upper_limit_timesteps_training=ts+1)
                        ratio = torch.mean((x0_estimate - target_frame)**2)/torch.mean((x0_estimates_clean[model.timesteps-1-ts] - target_frame)**2)
                        if ratio > 1.04:
                            print(model.sqrtAlphasCumprod.ravel()[ts], model.sqrtAlphasCumprod.ravel()[ts])
                            mse_input_type2.append(torch.mean((input_data - target_frame)**2))
                            break
                        if ratio <= 1.04 and i == 49:
                            mse_input_type2.append(torch.mean((input_data - target_frame)**2))"""

                mse_input = mse_input[::-1]
                #mse_input_type2 = mse_input_type2[::-1]
                ...
                mse_input_all[name].append(mse_input)
                #mse_input_type2_all[name].append(mse_input_type2)
                shortcuts_all[name].append(shortcuts)
    ...
    for name in models:
        ...
        mean_mse_input[name] = torch.mean((torch.tensor(mse_input_all[name])), dim=0)
        #mean_mse_input_type2[name] = torch.mean((torch.tensor(mse_input_type2_all[name])), dim=0)
        mean_shortcuts[name] = torch.mean((torch.tensor(shortcuts_all[name])).float(), dim=0)

    return {
        "mse_ancestor": mean_mse_ancestor,   # (N_total, T, C, H, W)
        "mse_clean": mean_mse_clean,   # (N_total, T, C, H, W)
        "mse_input": mean_mse_input,
        #"mse_input_type2": mean_mse_input_type2,
        "shortcuts": mean_shortcuts
    }
...
def main():
    ...
    fig, axes = plt.subplots(2, 2, figsize=(3*len(models), 6), sharex=True)
    ...
    for i, model_name in enumerate(models):
        ...
        mse_input = results['mse_input'][model_name]
        #mse_input_type2 = results['mse_input_type2'][model_name]
        shortcuts = results['shortcuts'][model_name].long()
        alphas = list(models[model_name].sqrtOneMinusAlphasCumprod.ravel().cpu())[::-1]

        axes[0,0].hist(alphas, alpha=0.2, color=colors[i], bins=np.logspace(-2.5, 0, 20))

        # Plot curves
        axes[1,0].plot(alphas, mse_clean,
                    label="Training input", color=colors[i], linestyle='dotted')
        axes[1,0].plot(alphas, mse_input,
                    label="Inference input", color=colors[i])
        #axes[1,0].plot(alphas, mse_input_type2,
        #            label="Inference input Type 2", color=colors[i], linestyle='dashdot')

        # Grid and title
        axes[1,0].grid(True, which='both', linestyle='--', alpha=0.3)
        axes[0,0].set_title(model_name)
        ...
        index = 0
        while True:
            shortcut_listed.append(index)
            if index == 99:
                break
            next_index = shortcuts[99-index]
            if index == next_index:
                index = next_index+1
            else:
                index = next_index
        print(shortcut_listed)
